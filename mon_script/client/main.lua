...
local chronoThread = nil
local chronoStartTime = 0

local function formatChronoTime(ms)
...
local function startChrono()
    if chronoRunning then
        return
    end

    chronoRunning = true
    chronoStartTime = GetGameTimer()

    chronoThread = CreateThread(function()
        while chronoRunning do
            local elapsedTime = GetGameTimer() - chronoStartTime
            DrawTxt("Chrono: " .. formatChronoTime(elapsedTime), 0.5, 0.05)
            Wait(0)
        end
        chronoThread = nil
    end)
...
local function stopChrono()
    if not chronoRunning then
        return
    end
    ...
    local elapsedTime = GetGameTimer() - chronoStartTime
    message("Chrono arrêté: " .. formatChronoTime(elapsedTime))
end
...
RegisterKeyMapping("startchrono", "Démarrer le chrono", "keyboard", "E")

RegisterNetEvent("mon_script:client:startChrono", function()
    startChrono()
end)
...
RegisterKeyMapping("stopchrono", "Arrêter le chrono", "keyboard", "X")